...
echo -e "  -p [port1,port2,...]      Set ports (default: 443)"
...
function realip() {
    # echo $1
    for p in ${port//,/ }; do
        sparrow=$(curl -A "trace" --resolve cf-ns.com:$p:$1 https://cf-ns.com:$p/cdn-cgi/trace  -s --connect-timeout 2 --max-time 10 | grep "uag")
        if [ "$sparrow" == "uag=trace" ]; then
            echo $1 >> realip.txt
            break
        fi
    done
...
function cloudflarerealip() {
    ...
    for i in `cat $filename | tr -d '\r'`; do
        read -u6;
        {
            realip $i;
            echo >&6
...
function cloudflarertt() {
    if [! -f "realip.txt" ]; then
        ...
        n=1
        for i in `cat realip.txt | tr -d '\r'`; do
            read -u6;
            {
                rtt $i;
                echo >&6
...
if [! -f "rtt.txt" ]; then
    ...
    for i in `cat rtt.txt | sed -e '/ /_/g'`; do
